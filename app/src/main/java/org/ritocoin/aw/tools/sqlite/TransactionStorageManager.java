...
public class TransactionStorageManager {
    ...
    public static boolean putTransaction(Context app, String iso, BRTransactionEntity tx) {
        ...
        if (iso.equalsIgnoreCase("RITO")/* || iso.equalsIgnoreCase("bch")*/) {
            BRTransactionEntity result = RitoTransactionDataStore.getInstance(app).putTransaction(app, iso, tx);
            return result != null;
        }
    ...
    public static List<BRTransactionEntity> getTransactions(Context app, String iso) {
        ...
        if (iso.equalsIgnoreCase("RITO")/* || iso.equalsIgnoreCase("bch")*/) {
            return RitoTransactionDataStore.getInstance(app).getAllTransactions(app, iso);
        }
    ...
    public static boolean updateTransaction(Context app, String iso, BRTransactionEntity tx) {
        ...
        if (iso.equalsIgnoreCase("RITO")/* || iso.equalsIgnoreCase("bch")*/) {
            return RitoTransactionDataStore.getInstance(app).updateTransaction(app, iso, tx);
    ...
    public static boolean removeTransaction(Context app, String iso, String hash) {
        ...
        if (iso.equalsIgnoreCase("RITO")/* || iso.equalsIgnoreCase("bch")*/) {
            RitoTransactionDataStore.getInstance(app).deleteTxByHash(app, iso, hash);
            return true;
        }
        //other wallets
